...
class CartController extends Controller
{
    ...
    public function updateCart(Request $request)
    {
        $rowId = $request->rowId;
        $qty = $request->qty;
        $requestPrice = $request->request_price;

        // Update cart item with new quantity and request price if set
        $item = Cart::get($rowId);
        $price = $requestPrice ? $requestPrice : $item->price;
        Cart::update($rowId, ['qty' => $qty, 'price' => $price]);

        // Recalculate subtotal and total
        $subtotal = Cart::subtotal();
        $total = Cart::total();
        $itemPrice = Cart::get($rowId)->price * $qty;
    ...
    public function deleteItem(Request $request)
    {
        ...
        $subtotal = Cart::subtotal();
        $total = Cart::total();
        $cartIsEmpty = Cart::count() === 0;

        return response()->json([
            'success' => true,
            'subtotal' => (float) str_replace(',', '', $subtotal),
            'total' => (float) str_replace(',', '', $total),
            'cart_is_empty' => $cartIsEmpty,
        ]);
    }
    ...
    {
        // Loop through the cart items and update their prices
        foreach ($request->cartItems as $item) {
            Cart::update($item['rowId'], ['price' => $item['request_price']]);
        }
